...
/**
 * ICE / TURN config for WebRTC. Credentials stay on the server; clients call
 * {@code GET /calls/ice-servers}.
 *
 * <p>Production: set {@code turn-urls} + either static {@code turn-username}/
 * {@code turn-credential} or coturn REST {@code turn-shared-secret}.
 * Dev: if no TURN configured and {@code allow-dev-openrelay=true}, falls back to
 * Metered openrelay (emulator-friendly only — never enable in prod).
 */
...
public class WebRtcIceProperties {
    ...
    /**
     * Coturn REST API shared secret. When set (and turn-urls non-empty), issues
     * time-limited username/password instead of static credentials.
     */
    ...
    /**
     * Dev-only fallback to public openrelay when no TURN is configured.
     * Must stay false in production.
     */
    private boolean allowDevOpenrelay = false;

    /**
     * Append Metered openrelay as an extra TURN server alongside configured TURN.
     * Helps emulator QA when coturn relay is unreachable; keep false in hardened prod.
     */
