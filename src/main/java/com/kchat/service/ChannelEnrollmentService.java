...
public class ChannelEnrollmentService {
  ...
  static final List<DefaultChannel> DEFAULT_CHANNELS =
      List.of(
          new DefaultChannel("ops-alerts", "ops-alerts"), new DefaultChannel("support", "support"));
  ...
  private ChatRoom findOrCreateChannel(DefaultChannel def, List<User> activeUsers) {
    ...
    try {
      ...
      return chatRoomRepository.findBySlug(def.slug()).orElseThrow(() -> ex);
    }
  }

  private void enrollMissingMembers(ChatRoom room, List<User> activeUsers) {
    boolean hasOwner =
        roomMemberRepository.findActiveMembersWithUser(room.getId()).stream()
    ...
    roomMemberRepository
        ...
        .ifPresentOrElse(
            existing -> {
              ...
              if (RoomRoles.OWNER.equals(preferredRole)
                  && !RoomRoles.OWNER.equals(existing.getRole())) {
