...
public class MessagePushDeliveryService {
    ...
    try {
      ...
      DirectRoomPair pair =
          room.getType() == RoomType.direct
              ? directRoomPairRepository.findByRoomIdIn(List.of(roomId)).stream()
                  .findFirst()
                  .orElse(null)
              : null;
      ...
      for (UUID memberId : memberIds) {
        ...
        RoomMember membership =
            roomMemberRepository.findActiveMembership(roomId, memberId).orElse(null);
        if (membership == null) {
          continue;
        }
        ...
        if (!pushDeliveryPolicy.shouldSendRoomMessagePush(settings, membership, now, memberId)) {
          log.info(
              "Push skipped for member {}: pushEnabled={}, quietHours/mute active",
              memberId,
        ...
        List<UserDevice> devices =
            userDeviceRepository.findByUser_IdOrderByLastActiveAtDesc(memberId);
        int sentCount = 0;
        ...
        if (sentCount == 0) {
          log.info(
              "No valid FCM push tokens for recipient {} (total devices: {})",
              memberId,
              devices.size());
        }
      }
  ...
  private static String formatBody(MessageDto message) {
    ...
    return switch (type) {
      ...
      case "system" ->
          message.botTitle() == null || message.botTitle().isBlank() ? "[Bot]" : message.botTitle();
      default -> {
        String text = message.text();
        if (text == null || text.isBlank()) {
          yield "Tin nhắn mới";
  ...
  private String fallbackPeerTitle(UUID roomId, UUID userId) {
    ...
    return shown.stream()
        .map(MessagePushDeliveryService::displayName)
