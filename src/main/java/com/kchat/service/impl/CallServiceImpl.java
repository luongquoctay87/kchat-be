...
public class CallServiceImpl implements CallService {
  ...
  public CallDto initiate(UUID userId, UUID roomId, String callTypeRaw) {
    ...
    log.info(
        "Call initiated: callId={} caller={} callee={} room={} type={}",
        call.getId(),
        userId,
        calleeId,
        roomId,
  ...
  public CallDto accept(UUID userId, UUID callId) {
    ...
    int updated =
        callSessionRepository.transitionStatus(callId, CallStatus.ringing, CallStatus.active, now);
    if (updated == 0) {
    ...
    List<UUID> recipients = new ArrayList<>(callParticipantRepository.findUserIdsByCallId(callId));
  ...
  public CallDto decline(UUID userId, UUID callId) {
    ...
    Instant now = Instant.now();
    int updated =
        callSessionRepository.endIfStatus(callId, CallStatus.ringing, CallStatus.declined, now);
    if (updated == 0) {
      throw ApiException.badRequest("call_state", "Call is not ringing");
    }
  ...
  public CallDto end(UUID userId, UUID callId) {
    ...
    CallStatus next;
    int updated =
        callSessionRepository.endIfStatus(
    ...
    if (updated > 0) {
      ...
      updated = callSessionRepository.endIfStatus(callId, CallStatus.active, CallStatus.ended, now);
      if (updated == 0) {
        throw ApiException.badRequest("call_state", "Call already ended");
      }
  ...
  public void relayIceOffer(UUID userId, UUID callId, String sdp) {
    ...
    log.info(
        "Relaying ICE offer for call {} from user {} to {} recipient(s)",
        callId,
        userId,
        others.size());
  ...
  public void relayIceAnswer(UUID userId, UUID callId, String sdp) {
    ...
    List<UUID> others = otherParticipants(callId, userId);
    log.info(
        "Relaying ICE answer for call {} from user {} to {} recipient(s)",
        callId,
        userId,
        others.size());
  ...
  public void relayIceCandidate(
    ...
    List<UUID> others = otherParticipants(callId, userId);
    log.info(
        "Relaying ICE candidate for call {} from user {} to {} recipient(s)",
        callId,
        userId,
        others.size());
  ...
  public int expireRingingCalls() {
    ...
    List<CallSession> expiredRinging =
        callSessionRepository.findByStatusAndCreatedAtBefore(CallStatus.ringing, ringCutoff);
    for (CallSession call : expiredRinging) {
      ...
      CallSession fresh =
          callSessionRepository.findByIdWithRoomAndInitiator(call.getId()).orElse(null);
      if (fresh == null) {
        continue;
      }
      ...
      List<UUID> recipients =
          new ArrayList<>(callParticipantRepository.findUserIdsByCallId(fresh.getId()));
    ...
    // Expire on startedAt (media start), not createdAt — otherwise long calls die at ~15m from
    // dial.
  ...
  private boolean supersedeCall(CallSession call) {
    ...
    CallSession fresh =
        callSessionRepository.findByIdWithRoomAndInitiator(call.getId()).orElse(call);
    CallDto dto = toDto(fresh, resolveCallee(fresh));
    List<UUID> recipients =
        new ArrayList<>(callParticipantRepository.findUserIdsByCallId(fresh.getId()));
  ...
  private void postCallEventMessage(CallSession call, String text) {
    ...
    UUID dtoViewer =
        memberIds.stream().filter(id -> !id.equals(initiatorId)).findFirst().orElse(initiatorId);
  ...
  private CallSession requireParticipantCall(UUID callId, UUID userId) {
    CallSession call =
        callSessionRepository
            .findByIdWithRoomAndInitiator(callId)
  ...
  private static String formatEndedSummary(CallSession call) {
    ...
    long seconds =
        Math.max(0, Duration.between(call.getStartedAt(), call.getEndedAt()).getSeconds());
  ...
  private void sendCallIncomingPush(CallSession call, UUID calleeId) {
    try {
      ...
      List<UserDevice> devices =
          userDeviceRepository.findByUser_IdOrderByLastActiveAtDesc(calleeId);
  ...
  private void sendCallEndedPush(UUID callId, UUID targetUserId, String reason) {
    try {
      ...
      List<UserDevice> devices =
          userDeviceRepository.findByUser_IdOrderByLastActiveAtDesc(targetUserId);
      for (UserDevice device : devices) {
        String token = device.getFcmToken();
        if (token == null || token.isBlank() || token.startsWith("dev:")) {
          continue;
