...
public class MessageRetentionCleanupServiceImpl implements MessageRetentionCleanupService {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(MessageRetentionCleanupServiceImpl.class);
  ...
  public int purgeMessagesOlderThan(Instant cutoff, int batchSize) {
    ...
    List<ExpiredMessageRow> old =
        chatMessageRepository.findOlderThanForRetentionCleanup(cutoff, limit);
    if (old.isEmpty()) {
      return 0;
    }
