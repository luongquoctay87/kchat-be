...
public class RedisCallEventPublisher implements CallEventPublisher {
    ...
    @Override
    public void iceOffer(String callId, String roomId, UUID fromUserId, String sdp, List<UUID> recipientIds) {
        publishIce(WsEnvelope.TYPE_ICE_OFFER, callId, roomId, fromUserId, sdp, null, null, null, recipientIds);
    }

    @Override
    public void iceAnswer(String callId, String roomId, UUID fromUserId, String sdp, List<UUID> recipientIds) {
        publishIce(WsEnvelope.TYPE_ICE_ANSWER, callId, roomId, fromUserId, sdp, null, null, null, recipientIds);
    }
    ...
    private void publishIce(
        ...
        WsEnvelope envelope = WsEnvelope.iceSignal(
                type,
    ...
    private void publish(UUID roomId, WsEnvelope envelope, String type) {
        try {
            redis.convertAndSend(RedisChannels.roomChannel(roomId), objectMapper.writeValueAsString(envelope));
        } catch (Exception ex) {
