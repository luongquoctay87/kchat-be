...
public class RedisMessageEventPublisher implements MessageEventPublisher {
    ...
    public void messageDeleted(UUID roomId, String messageId, List<UUID> memberIds) {
        ...
        WsEnvelope envelope = WsEnvelope.messageDeleted(
    ...
    public void messagesRead(
        ...
        WsEnvelope envelope = WsEnvelope.messagesRead(
    ...
    private void publishMessageEvent(
        ...
        WsEnvelope envelope = new WsEnvelope(
                type,
                new MessageNewPayload(roomId.toString(), senderId, memberIds, canonical)
        );
        try {
            redis.convertAndSend(RedisChannels.roomChannel(roomId), objectMapper.writeValueAsString(envelope));
        } catch (JsonProcessingException ex) {
