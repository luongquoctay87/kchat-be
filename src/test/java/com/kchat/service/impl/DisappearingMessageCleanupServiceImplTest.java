...
class DisappearingMessageCleanupServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        service = new DisappearingMessageCleanupServiceImpl(
    ...
    void purgeExpiredMessages_deletesDbThenMediaThenNotifies() {
        ...
        order.verify(messageEventPublisher).messageDeleted(roomId, messageId.toString(), List.of(memberId));
    }
    ...
    void purgeExpiredMessages_capsBatchSize() {
        ...
        verify(chatMessageRepository).findExpiredForDisappearingCleanup(
                DisappearingMessageCleanupServiceImpl.MAX_BATCH_SIZE
        );
    }
