...
class MailTemplateRendererTest {
  ...
  void rendersPasswordResetOtpHtmlAndText() {
    MailTemplateRenderer.RenderedMail rendered =
        mailTemplateRenderer.renderOtp(
            MailTemplateRenderer.OtpMail.passwordReset(
                "k-chat", "k-chat", "support@company.com", "Nguyễn Văn A", "742891", 30));
    ...
    MailTemplateRenderer.RenderedMail rendered =
        mailTemplateRenderer.renderOtp(
            MailTemplateRenderer.OtpMail.registration(
                "k-chat", "k-chat", "support@company.com", "Nguyễn Văn A", "123456", 30));

    assertThat(rendered.html()).contains("123456", "Xác minh đăng ký", "cid:kchatLogo");
    assertThat(rendered.text()).contains("123456", "Mã xác minh đăng ký");
  }
}
